...
def load_artist(file_name):
    ...
    artist = {
        ...
        "nombre": artist_xml.name.text,
        "nacionalidad": artist_xml.nationality.text
    }
    return artist
...
def load_song(file_name):
    ...
    song = {
        "id": song_xml.song["id"],
        "titulo": song_xml.title.text,
        "duracion": song_xml.duration.text,
        "genero": song_xml.genre.text,
        "artista": song_xml.artist.text
    }
    return song
...
def load_genre(file_name):
    ...
    genre = {
        ...
        "nombre": genre_xml.name.text,
        "descripcion": genre_xml.description.text
    }
    return genre
...
def show_menu_albums():
    ...
    while True:
        ...
        if opcion == "0":
            ...
            print("Mostrando todos los álbumes...")
            load_albums()
            for album in albums:
    ...
    while True:
        ...
        if opcion == "0":
            ...
            for artist in artists:
                print(f"{artist['id']} - {artist['nombre']} ({artist['nacionalidad']})")
        elif opcion == "2":
            nombre_artista = input("Introduce el nombre del artista que deseas buscar: ")
            load_artists()
            encontrados = [a for a in artists if nombre_artista.lower() in a['nombre'].lower()]
...
def show_menu_songs():
    ...
    while True:
        ...
        if opcion == "0":
            ...
            load_songs()
            for song in songs:
                print(f"{song['id']} - {song['titulo']} ({song['duracion']}) [{song['genero']}] - {song['artista']}")
        elif opcion == "2":
            titulo_cancion = input("Introduce el título de la canción que deseas buscar: ")
            load_songs()
            encontrados = [s for s in songs if titulo_cancion.lower() in s['titulo'].lower()]
